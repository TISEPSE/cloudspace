...
export default function SearchModal({ onClose, onOpenFile }) {
  ...
  useEffect(() => {
    if (query.length < 2) { setResults([]); setLoading(false); setSearchedQuery(''); return }
    setLoading(true)
    const timer = setTimeout(() => {
      apiFetch(`/api/search?q=${encodeURIComponent(query)}`)
        ...
        .catch(() => {})
        .finally(() => { setLoading(false); setSearchedQuery(query) })
    }, 250)
  ...
  const noResults = searchedQuery === query && searchedQuery.length >= 2 && !loading && results.length === 0
  const showBody = hasResults || noResults

  return (
    <div className="fixed inset-0 z-50" onClick={onClose}>
      <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" />
      <div
        className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[calc(100%-2rem)] max-w-lg bg-white dark:bg-surface-dark rounded-2xl border border-slate-200 dark:border-border-dark shadow-2xl overflow-hidden"
        onClick={e => e.stopPropagation()}
      >
        {/* Input — toujours en premier, ne bouge jamais */}
        ...
        {/* Container résultats — animé en max-height pour éviter tout glitch */}
        <div
          className="overflow-hidden transition-[max-height] duration-200 ease-out"
          style={{ maxHeight: showBody ? '320px' : '0px' }}
        >
          {hasResults && (
            <div className="overflow-y-auto py-1.5" style={{ maxHeight: '320px' }}>
              {results.map((item, i) => (
                <button
                  key={item.id}
