...
function FolderRow({ folder, onAction, onOpen }) {
  return (
    <div onClick={() => onOpen(folder.id)} className="flex items-center p-3 bg-white dark:bg-surface-dark border border-slate-200 dark:border-border-dark rounded-lg hover:bg-slate-50 dark:hover:bg-[#1f2d3d] cursor-pointer transition-colors shadow-sm group">
      ...
      <FileContextMenu isFolder isStarred={true} onAction={(id) => onAction(id, folder)} />
    </div>
  )
}
...
function FileCard({ file, onPreview, onAction, showExt }) {
  ...
  return (
    <div onClick={() => onPreview(file)} className="group relative bg-white dark:bg-surface-dark border border-slate-200 dark:border-border-dark rounded-lg p-2 hover:border-primary/50 dark:hover:border-primary/50 hover:shadow-md transition-all cursor-pointer">
      ...
      <div className="flex items-center">
        ...
        <FileContextMenu isStarred={true} onAction={(id) => onAction(id, file)} />
      </div>
    </div>
  )
}

function FileRowList({ file, onPreview, onOpen, onAction, showExt }) {
  const displayName = formatDisplayName(file.name, showExt)
  return (
    <tr onClick={() => file.is_folder ? onOpen(file.id) : onPreview(file)} className="group hover:bg-slate-50 dark:hover:bg-[#1f2d3d] transition-colors cursor-pointer">
      <td className="px-5 py-3">
        <div className="flex items-center gap-3">
          <div className={`w-8 h-8 rounded-lg ${file.icon_bg} flex items-center justify-center flex-shrink-0`}>
            <span className={`material-symbols-outlined text-xl ${file.icon_color}`} style={file.is_folder ? { fontVariationSettings: "'FILL' 1" } : undefined}>
    ...
    <div className="flex-1 overflow-y-auto p-3 sm:p-6 flex flex-col">
      ...
      {isEmpty && (
        <div className="flex-1 flex flex-col items-center justify-center text-center">
          ...
          <p className="text-sm text-slate-400 dark:text-slate-500 mt-1">Les fichiers et dossiers que vous marquez apparaîtront ici.</p>
        </div>
      )}
        ...
        <>
          {folders.length > 0 && (
            <section className="mb-8">
              <h3 className="text-xs font-bold text-slate-400 mb-3 uppercase tracking-wider">Dossiers</h3>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-2">
                {folders.map(f => <FolderRow key={f.id} folder={f} onAction={handleAction} onOpen={(id) => navigate(`/drive/folder/${id}`)} />)}
              </div>
            </section>
          )}
          {files.length > 0 && (
            <section>
              <h3 className="text-xs font-bold text-slate-400 mb-3 uppercase tracking-wider">Fichiers</h3>
              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-7 gap-2">
                {files.map(f => <FileCard key={f.id} file={f} onPreview={setPreviewFile} onAction={handleAction} showExt={showExt} />)}
              </div>
            </section>
          )}
        </>
        ...
        <div className="bg-white dark:bg-surface-dark rounded-xl border border-slate-200 dark:border-border-dark overflow-hidden shadow-sm">
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-50 dark:bg-[#151e26] border-b border-slate-200 dark:border-border-dark">
              <tr>
                <th className="px-5 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider w-[50%]">Nom</th>
                <th className="hidden md:table-cell px-5 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider w-[25%]">Modifié</th>
                <th className="hidden sm:table-cell px-5 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider w-[20%]">Taille</th>
                <th className="px-5 py-3 w-[5%]"></th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-border-dark">
              {[...folders, ...files].map(item => (
                <FileRowList key={item.id} file={item} onPreview={setPreviewFile} onOpen={(id) => navigate(`/drive/folder/${id}`)} onAction={handleAction} showExt={showExt} />
              ))}
            </tbody>
          </table>
        </div>
